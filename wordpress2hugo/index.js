...
tds.addRule('attribution', {
    filter: (node, options) => {
        return (node.nodeName == "DIV" && node.className == "attribution");
    },
    replacement: function (content) {
    ...
    fs.readFile(backupXmlFile, function (err, data) {
        parser.parseString(data, function (err, result) {
            ...
            posts = posts.filter(function (post) {
                let status = post["wp:status"] && post["wp:status"].join('');
                let type = post["wp:post_type"] && post["wp:post_type"].join('');

                return status != "private" && status != "inherit" && type != "feedback" && type != "page";
            });
            ...
            posts.forEach(function (post) {
                var postMap = {};
                let type = post["wp:post_type"] ? post["wp:post_type"][0] : "post";
                ...
                slug = post["wp:post_name"] ? post["wp:post_name"][0] : undefined;
                ...
                if (categories && categories.length) {
                    categories.forEach(function (category) {
                        ...
                        let tag = category['_'].replace(/'/g, "''");

                        if (tag !== "Uncategorized" || categories.length == 1) {
                            tags.push(tag);
                        }
                    });
                ...
                if (tags.indexOf("Citation") > -1) {
                    type = "quote";
                }
                // let postDir = outputDir + '/' + postType;
                ...
                if (post["content:encoded"]) {
                    ...
                    var postContent = post["content:encoded"].toString().trim();
                    ...
                    // Twitter shortcode
                    // markdown = markdown.replace(/([^\(\w]|^)https?:\/\/(www\.)?twitter.com\/([^/]+)\/status\/(\d+)\/?/, '$1{{< tweet user="$3" id="$4" >}}');

                    // Youtube shortcode
                    markdown = markdown.replace(/\\\[youtube https:\/\/([\w\.]+)?youtube\.(\w+)\/watch\?(.+&)?v=([\w-]+)[^\]]*\\\]/, '{{< youtube $4 >}}')
                    markdown = markdown.replace(/([^\(\w]|^)https?:\/\/([\w\.]+)?youtube\.(\w+)\/watch\?(.+&)?v=([\w-]+)([^\(\w]|$)*/, '$1{{< youtube $5 >}}$6')

                    // Vimeo shortcode
                    markdown = markdown.replace(/\\\[vimeo (clip\\_id=")?(\d+)"?[^\]]*\\\]/, '{{< vimeo $2 >}}');
                    markdown = markdown.replace(/([^\(\w]|^)https?:\/\/([\w\.]+)?vimeo\.(\w+)\/(\d+)/, '$1{{< vimeo $4 >}}');

                    // Code shortcode
                    markdown = markdown.replace(/\\\[\/?code ?(language="(\w+)")?\\\]/gm, '\n```$2\n');
                    markdown = markdown.replace(/\\\[\/?code ?(language="(\w+)")? highlight="(\d+)"\\\]/gm, '\n```$2 {hl_lines=[$3]}\n');
